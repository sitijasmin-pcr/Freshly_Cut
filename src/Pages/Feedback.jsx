...
export default function FormFeedback() {
  ...
  const handleSubmit = (e) => {
    ...
    setForm({ name: "", email: "", rating: "", feedback: "", role: "", job: "", photoUrl: "" });
          ...
          <div key={i} className="relative group">
            ...
            <div className="relative z-10 bg-white rounded-2xl shadow-xl p-6 text-center border border-orange-100">
              <div className="-mt-16 mb-4">
                <img
                  src={item.photoUrl || `https://i.pravatar.cc/100?img=${i + 1}`}
                  alt={item.name}
              ...
              <div className="flex justify-center gap-2 text-sm font-medium text-orange-500 mb-2">
                ...
                {item.job && <span className="text-gray-500">| {item.job}</span>}
              </div>
              <p className="text-gray-700 text-sm italic mb-4">"{item.feedback}"</p>
                ...
                {[...Array(5)].map((_, index) => (
                  <div
                    ...
                    className={`rounded-full w-12 h-12 flex items-center justify-center shadow-md bg-white border ${
                      index < parseInt(item.rating) ? "text-yellow-400" : "text-gray-300"
                    } z-${5 - index} relative`}
                    style={{
                  ...
                  >
                    <Star size={20} className={index < parseInt(item.rating) ? "fill-yellow-400" : ""} />
                  </div>
                ))}
              </div>
            </div>
      ...
      <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="w-full max-w-lg rounded-xl bg-white p-6 relative">
            ...
            <Dialog.Title className="text-lg font-bold mb-4 text-orange-700">Form Feedback</Dialog.Title>

            <form onSubmit={handleSubmit} className="space-y-4">
              <Input label="Nama Lengkap" name="name" value={form.name} onChange={handleChange} required />
              <Input label="Email (opsional)" name="email" type="email" value={form.email} onChange={handleChange} />
              <Select label="Peran" name="role" value={form.role} onChange={handleChange} required>
                <option value="">Pilih peran Anda</option>
                <option value="Bronze">Bronze</option>
                <option value="Silver">Silver</option>
                <option value="Gold">Gold</option>
              </Select>
              <Input label="Pekerjaan" name="job" value={form.job} onChange={handleChange} />
              <Input label="Link Foto Profil (opsional)" name="photoUrl" value={form.photoUrl} onChange={handleChange} />
              <Select label="Rating Pelayanan" name="rating" value={form.rating} onChange={handleChange} required>
              ...
              <Textarea label="Masukan atau Saran" name="feedback" value={form.feedback} onChange={handleChange} required />
              <div className="flex justify-end">
                <Button type="submit" className="bg-orange-600 hover:bg-orange-700">Kirim</Button>
              </div>
            </form>
          </Dialog.Panel>
        </div>
