...
//           >
//             <i className="fas fa-times-circle mr-2"></i> Batalkan
//           </button>
//         )}
//         <button
//           type="submit"
...
const UserForm = ({ addUser, updateUser, editingUser, setEditingUser }) => {
  const [form, setForm] = useState({ nama: '', email: '', pass: '' });

  useEffect(() => {
    if (editingUser) {
      setForm({ nama: editingUser.nama, email: editingUser.email, pass: '' });
    } else {
      setForm({ nama: '', email: '', pass: '' });
    }
  }, [editingUser]);

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.nama || !form.email || (!editingUser && !form.pass)) {
      alert('Nama, Email, dan Password wajib diisi!');
      return;
    }
    editingUser ? updateUser({ id: editingUser.id, ...form }) : addUser(form);
    setForm({ nama: '', email: '', pass: '' });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {['nama', 'email', 'pass'].map((field) => (
          <div key={field}>
            <label className="block text-xs font-black uppercase opacity-50 mb-1">
              {field === 'pass' ? 'Password' : field}
            </label>
            <input
              type={field === 'pass' ? 'password' : field === 'email' ? 'email' : 'text'}
              name={field}
              value={form[field]}
              onChange={(e) => setForm({ ...form, [field]: e.target.value })}
              placeholder={field === 'pass' && editingUser ? "Kosongkan jika tidak ubah" : `Masukkan ${field}`}
              className="w-full p-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-[#004d33] focus:outline-none transition-all font-bold"
              required={field !== 'pass' || !editingUser}
            />
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-end gap-3 pt-2">
        {editingUser && (
          <button type="button" onClick={() => setEditingUser(null)} className="px-8 py-4 rounded-2xl font-black text-gray-400 hover:bg-gray-100">Batal</button>
        )}
        <button type="submit" className="px-8 py-4 bg-[#004d33] text-white rounded-2xl font-black hover:opacity-90 transition-all">
          {editingUser ? 'Perbarui Data' : 'Tambah Pengguna'}
        </button>
      </div>
    </form>
  );
