...
const Dashboard = () => {
  ...
  const fetchDashboardData = useCallback(async (period) => {
    ...
    try {
      ...
      // 2. Ambil data pelanggan untuk pemetaan nama
      // Catatan: Karena 'orders' tidak memiliki 'customer_id', kita hanya bisa mengandalkan 'customer_name' dari orders.
      // Gambar profil pelanggan akan dibuat dengan DiceBear.
      const { data: customersData, error: customersError } = await supabase
        .from('customers')
        .select('id, nama'); // Mengambil 'id' dan 'nama' untuk validasi dan referensi
      if (customersError) throw customersError;
      // const customerMap = new Map(customersData.map(c => [c.id, c])); // Jika orders punya customer_id
      const customerNameMap = new Map(customersData.map(c => [c.nama, c.id])); // Untuk validasi nama pelanggan jika diperlukan
      ...
      ordersData.forEach(order => {
        ...
        order.order_items.forEach(item => {
          ...
          // Gunakan nama dari tabel 'produk' jika ada, jika tidak, gunakan dari 'order_items'
          const productName = productInfo?.nama || item.product_name;
          // Gunakan gambar dari tabel 'produk' jika ada, jika tidak, gunakan placeholder
          ...
          // Hitung profit berdasarkan price_per_unit dari order_items
        ...
        if (customerName) {
          // Placeholder gambar pelanggan dengan DiceBear Avatars
          const customerImageUrl = `https://api.dicebear.com/8.x/initials/svg?seed=${customerName}&radius=50&backgroundColor=008080,FB923C&size=40`;
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      <div className="p-6 lg:p-8">
        ...
        <div className="flex space-x-2 mb-6">
          ...
          <button className="px-5 py-2 bg-white text-gray-700 rounded-lg font-medium shadow-md hover:bg-gray-100 transition-all duration-200">Performa</button>
          <button className="px-5 py-2 bg-white text-gray-700 rounded-lg font-medium shadow-md hover:bg-gray-100 transition-all duration-200">Kampanye Iklan</button>
        </div>
