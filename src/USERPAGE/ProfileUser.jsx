...
import { ShoppingCart, Bell, UserCircle, LogOut, Edit3, Loader2 } from "lucide-react";
import { supabase } from "../supabase";
...
export default function ProfileUser() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        setLoading(true);
        // Mengambil data berdasarkan email dan memastikan role adalah customer
    ...
    <div className="font-sans text-gray-900 bg-[#F0F9FF] min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-10">
            {["Home", "Menu", "Story", "FAQ", "Feedback"].map((item) => (
              <Link key={item} to={item === "Home" ? "/HomeUser" : `/${item}User`} className="text-sm font-bold uppercase tracking-widest text-gray-500 hover:text-orange-600">
                {item}
              </Link>
            ))}
          </nav>
          <div className="flex items-center gap-5">
            <Link to="/ProfileUser" className="p-2 bg-gray-50 rounded-full text-blue-600"><UserCircle size={22} /></Link>
      ...
      <main className="py-24 px-6">
        <div className="max-w-2xl mx-auto">
          <FadeInOnScroll>
            <div className="bg-white rounded-[2rem] border-b-8 border-blue-100 shadow-xl p-8 md:p-12">
              {loading ? (
                <div className="flex flex-col items-center py-20 text-blue-600">
                  <Loader2 className="w-12 h-12 animate-spin mb-4" />
                  <p className="font-black italic">Loading Profile...</p>
                </div>
              ) : profile ? (
                <>
                  <div className="flex flex-col items-center mb-10">
                    <div className="w-32 h-32 rounded-full border-4 border-blue-100 mb-6 overflow-hidden shadow-lg">
                    ...
                    <h1 className="text-3xl font-black text-blue-900 uppercase italic">{profile.nama}</h1>
                  ...
                  <div className="mt-12 flex gap-4">
                    <button className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-orange-600 transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-200">
                      <Edit3 size={18} /> Edit Profile
                    </button>
                    <button onClick={logout} className="px-6 py-4 bg-gray-100 text-gray-500 rounded-2xl font-black hover:bg-red-50 hover:text-red-600 transition-all">
                      <LogOut size={18} />
                    </button>
                  </div>
                </>
              ) : (
                <div className="text-center text-blue-900 font-bold">Profil tidak ditemukan.</div>
              )}
            </div>
          </FadeInOnScroll>
        </div>
      ...
      <footer className="bg-white pt-24 pb-12 px-6 border-t border-gray-100">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12 border-b border-gray-100 pb-16 mb-8">
            {/* Footer content sama seperti HomeUser */}
             <div>
                <img src="src/assets/img/Logo buah segar _Freshly Cut_.png" alt="Logo" className="h-16 mb-6" />
                <p className="text-gray-400 text-sm font-medium italic">Empowering everyone to enjoy a high-quality cup. Freshness guaranteed.</p>
             </div>
             <div>
             ...
             {/* Tambahkan kolom lainnya sesuai HomeUser */}
        </div>
        <p className="text-center text-[10px] font-black text-gray-400 uppercase tracking-[0.5em]">&copy; 2026 FRESHLY CUT - ALL RIGHTS RESERVED</p>
      </footer>
    </div>
  );
}
...
function InfoRow({ label, value }) {
  return (
    <div className="border-b border-gray-100 pb-4">
      ...
      <p className="font-bold text-blue-900 mt-1">{value}</p>
    </div>
  );
}